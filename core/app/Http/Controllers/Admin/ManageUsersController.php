...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalAutionCreated = Auction::where('auction_owner', $id)->count();
        $totalAuctionBuy = Auction::where('auction_buyer', $id)->count();
        $totalDeposit = Deposit::where('user_id',$user->id)->where('status',1)->sum('amount');
        $totalWithdraw = Withdrawal::where('user_id',$user->id)->where('status',1)->sum('amount');
        $totalTransaction = Transaction::where('user_id',$user->id)->count();
        $totalCoinLog = CoinHistory::where('user_id', $user->id)->count();
        $totalTicket = SupportTicket::where('user_id', $id)->count();
        return view('admin.users.detail', compact('page_title', 'user','totalDeposit','totalWithdraw','totalTransaction', 'totalCoinLog', 'totalAutionCreated', 'totalAuctionBuy', 'totalTicket'));
    }
    ...
    public function coinLogs($id){
        User::findOrFail($id);
        $page_title = 'Coin Transaction Histories';
        $empty_message = 'There is no data';
        $logs = CoinHistory::where('user_id', $id)->latest()->paginate(getPaginate());
        return view('admin.users.coinLogs', compact('page_title', 'empty_message', 'logs'));
    }

    public function auctionCreated($id){
        $page_title = 'Created Auctions';
        $histories = Auction::where('auction_owner', $id)->latest()->paginate(getPaginate());
        $empty_message = 'There is no data';
        return view('admin.users.auctionLog', compact('page_title', 'empty_message', 'histories'));
    }

    public function boughtAuction($id){
        $page_title = 'Bought Auctions';
        $histories = Auction::where('auction_buyer', $id)->latest()->paginate(getPaginate());
        $empty_message = 'There is no data';
        return view('admin.users.auctionLog', compact('page_title', 'empty_message', 'histories'));
    }
    ...
    public function ticket($id){
        ...
        $items = SupportTicket::where('user_id', $id)->latest()->paginate(getPaginate());
        return view('admin.support.tickets', compact('items', 'page_title','empty_message'));
    }

    public function referrals($id){
        $user = User::findOrFail($id);
        $page_title = 'Referrals - '.$user->username;
        $referrals = User::where('ref_by',$id)->paginate(getPaginate());
        return view('admin.users.referrals', compact('page_title', 'referrals'));
    }
