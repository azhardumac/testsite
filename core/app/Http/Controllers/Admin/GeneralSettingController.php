...
class GeneralSettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $request->merge(['withdraw_permission' => isset($request->withdraw_permission) ? 1 : 0]);
        $request->merge(['registration' => isset($request->registration) ? 1 : 0]);

        $general_setting->update($request->only(['sitename', 'cur_text', 'cur_sym', 'coin_name', 'coin_symbol', 'withdraw_permission', 'ev', 'en', 'sv', 'sn', 'registration', 'base_color', 'secondary_color', 'depo_com']));
